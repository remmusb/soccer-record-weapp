<template>
  <view class="container">
    <!-- 加载状态 -->
    <view v-if="loading" class="loading">正在检查用户信息...</view>
    ...
    <view v-else-if="myPlayer" class="card">
      ...
      <view class="form-group">
        ...
        <view class="position-grid">
          <view class="position-item" v-for="pos in POSITIONS" :key="pos.id" :class="{'selected': form.positions.includes(pos.id)}" @click="togglePosition(pos.id)">
            <text>{{pos.icon}}</text>
            <text>{{pos.name}}</text>
          </view>
        </view>
    ...
    <!-- 管理员区域：添加其他球员 -->
    <view v-if="isAdmin" class="card admin-section">
      ...
      <view class="form-group">
        <text class="form-label">擅长位置</text>
        <view class="position-grid">
          <view class="position-item" v-for="pos in POSITIONS" :key="pos.id" :class="{'selected': adminForm.positions.includes(pos.id)}" @click="toggleAdminPosition(pos.id)">
            <text>{{pos.icon}}</text>
            <text>{{pos.name}}</text>
          </view>
        </view>
...
export default {
  ...
  methods: {
    ...
    togglePosition(id) {
      ...
      if (idx >= 0) this.form.positions.splice(idx, 1);
      else this.form.positions.push(id);
    },
    toggleAdminPosition(id) {
      const idx = this.adminForm.positions.indexOf(id);
      if (idx >= 0) this.adminForm.positions.splice(idx, 1);
      else this.adminForm.positions.push(id);
    },
